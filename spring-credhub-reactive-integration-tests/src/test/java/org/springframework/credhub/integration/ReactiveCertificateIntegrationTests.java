...
public class ReactiveCertificateIntegrationTests extends ReactiveCredHubIntegrationTests {
	...
	public void regenerateCertificate() {
		...
		StepVerifier.create(this.certificates.regenerate(certificateId.get(), true)).assertNext(response -> {
			assertThat(response.getName().getName()).isEqualTo(TEST_CERT_NAME.getName());
			assertThat(response.isTransitional()).isTrue();
			assertThat(response.getValue().getCertificate()).isNotEqualTo(certificate.get().getCertificate());
			assertThat(response.getValue().getCertificateAuthority())
					.isNotEqualTo(certificate.get().getCertificateAuthority());
			assertThat(response.getValue().getPrivateKey()).isNotEqualTo(certificate.get().getPrivateKey());
		}).verifyComplete();
	}
	...
	public void rotateCertificate() {
		...
		StepVerifier.create(this.credentials.getByNameWithHistory(TEST_CERT_NAME, CertificateCredential.class))
		...
		StepVerifier
				.create(this.certificates.updateTransitionalVersion(certificateId.get(), credentialVersion0Id.get()))
				.assertNext(response -> {
		...
		StepVerifier.create(this.certificates.updateTransitionalVersion(certificateId.get(), null))
				.assertNext(response -> {
					assertThat(response.getId()).isEqualTo(credentialVersion1Id.get());
					assertThat(response.isTransitional()).isFalse();
				}).verifyComplete();
	...
	public void bulkRegenerateCertificates() {
		...
		StepVerifier
				.create(this.credentials.generate(
						CertificateParametersRequest.builder().name(ROOT_CERT_NAME)
								.parameters(CertificateParameters.builder().commonName("example.com")
										.certificateAuthority(true).selfSign(true).build())
								.build(),
		...
		StepVerifier.create(this.credentials.generate(
				CertificateParametersRequest.builder().name(TEST_CERT_NAME)
						.parameters(CertificateParameters.builder().commonName("example.com")
								.certificateAuthorityCredential(ROOT_CERT_NAME).build())
						.build(),
				CertificateCredential.class)).assertNext(response -> {
					assertThat(response.getValue().getCertificateAuthority())
		...
		StepVerifier.create(this.certificates.regenerate(ROOT_CERT_NAME))
				.assertNext(response -> assertThat(response).isEqualTo(TEST_CERT_NAME)).verifyComplete();
	}
